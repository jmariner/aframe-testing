// https://raw.githubusercontent.com/supermedium/aframe-watcher/master/index.js
...
module.exports.setup = (app, options) => {
	options = Object.assign({ folders: [], prompt: true }, options);
	const files = getWorkingFiles(options.folders);
	app.use(cors());
 ...
 */
function getWorkingFiles(folders) {
	let files = [];

	folders.forEach(function (val, index, array) {
